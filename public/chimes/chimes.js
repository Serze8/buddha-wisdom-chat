...
export class StringChimes {
  ...
  // Build the audio graph (a suspended context is fine). Split from resume()
  // so the context can be created eagerly at page load: creating AND resuming
  // inside the same first gesture is unreliable and left the opening strikes
  // silent until a later interaction (e.g. switching country).
  warm() {
    ...
    // Prime the output with a 1-sample silent buffer so the first real strike
    // isn't swallowed by the context's cold-start latency.
    const prime = this.ctx.createBufferSource();
    prime.buffer = this.ctx.createBuffer(1, 1, this.ctx.sampleRate);

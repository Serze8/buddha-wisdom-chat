...
export const COUNTRIES = {
  ...
  india: {
    ...
    writing: "horizontal",
    gridW: 40,
    dense: true, // Devanagari collapses many code points into few grapheme cells; drop word-spaces so the curtain stays a tight wall (like CJK) instead of showing holes
...
// Split on grapheme clusters, not UTF-16 units, so a Devanagari/Arabic base
// letter keeps its combining matras (else an orphaned mark renders as ◌).
// Cached per cloth string — only ~13 distinct clothes ever pass through.
const graphemeSeg =
...
export function graphemesOf(text, dense = false) {
  const key = dense ? "\x00" + text : text;
  let cells = graphemeCache.get(key);
  if (!cells) {
    cells = graphemeSeg
      ? Array.from(graphemeSeg.segment(text), (s) => s.segment)
      : Array.from(text); // ponytail: code-point fallback for pre-Segmenter browsers; still beats UTF-16 units
    // `dense` drops whitespace so grapheme-collapsing scripts (Devanagari) pack
    // tight instead of leaving holes where word-spaces landed. Off by default so
    // space-separated Latin curtains keep their word breaks.
    if (dense) cells = cells.filter((g) => g.trim() !== "");
    graphemeCache.set(key, cells);
  }
  return cells;
}

export function charForCell(text, i, j, gridW, gridH, writing = "horizontal", dense = false) {

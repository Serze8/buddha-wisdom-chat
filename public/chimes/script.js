...
function applyCountryCopy(country) {
  ...
  document.title = `Chimes — ${country.name} · every country is a door | Niyam Vora`;
...
pane.hidden = true;
pane.element.classList.add("chimes-pane");
...
function main() {
  ...
  const charCanvases = {};
  for (const ch of new Set(graphemesOf(fullCode, country.dense))) {
    if (ch === " " || ch === "　") continue;
  ...
  for (let i = 0; i < gridW; i++) {
    for (let j = 0; j < gridH; j++) {
      ...
      const pinned = j === 0;
      const char = charForCell(fullCode, i, j, gridW, gridH, writing, country.dense);
...
function createContributionsCloth(host, names, cssW, cssH, gridFn) {
  ...
  const charCanvases = {};
  for (const ch of new Set(graphemesOf(fullCode))) {
    if (ch === " " || ch === "　") continue;
    const size = Math.ceil(fontSize * 1.35);
    const off = document.createElement("canvas");
    off.width = Math.ceil(size * dpr);
...
// Build the audio graph eagerly at load (suspended) so the first strike only
// has to RESUME an existing context — creating + resuming in the same first
// gesture is unreliable and left the opening strikes silent until a later click.
chimes.warm();
// Resume on user gestures; keep listening (capture-phase, runs before the string
// handler) until the context is actually running, then detach.
const warmChimes = () =>
  chimes.ensure().then((running) => {
    if (running) {
      window.removeEventListener("pointerdown", warmChimes, true);
      window.removeEventListener("touchstart", warmChimes, true);
      window.removeEventListener("keydown", warmChimes, true);
    }
  });
window.addEventListener("pointerdown", warmChimes, { capture: true });
window.addEventListener("touchstart", warmChimes, { capture: true, passive: true });
window.addEventListener("keydown", warmChimes, { capture: true });
updateSideButtons();
initCarousel();
initViews();
bindAreaCopyLayout();

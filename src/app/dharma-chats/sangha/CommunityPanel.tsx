...
type PostRow = Post & { profiles?: { display_name: string | null } | null }
...
export default function CommunityPanel() {
  ...
  const formatDate = (dateStr: string) => {
    return new Date(dateStr).toLocaleDateString(locale, { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })
  }

  return (
    <div>
      <h2 className="font-[var(--font-cormorant)] text-2xl font-bold text-amber-900 dark:text-amber-700 text-center mb-8">
        {locale === 'ru' ? 'Сангха — Общий чат' : 'Sangha — Community Chat'}
      </h2>
      {userId && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-900 rounded-2xl shadow border border-gray-200 dark:border-gray-700 p-6 mb-8 space-y-4">
          <input
            ...
            className="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-amber-800 dark:text-amber-600 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none"
            required
          />
          <textarea
            value={newContent}
            ...
            className="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-amber-800 dark:text-amber-600 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none resize-none"
            rows={3}
          />
          <input
            type="url"
        ...
        {posts.map((post) => (
          <div key={post.id} className="bg-white dark:bg-gray-900 rounded-2xl shadow border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-full bg-amber-200 dark:bg-amber-800 flex items-center justify-center text-amber-700 dark:text-amber-600 font-bold">
                {(post as PostRow).profiles?.display_name?.[0]?.toUpperCase() || '?'}
              </div>
              <div>
                <p className="font-medium text-amber-800 dark:text-amber-600 text-sm">
                  {(post as PostRow).profiles?.display_name || 'Anonymous'}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{formatDate(post.created_at)}</p>
              </div>
            </div>
            <h3 className="font-bold text-amber-800 dark:text-amber-600 mb-2">{post.title}</h3>

...
export default function HeroSilkAtlas() {
  ...
  useEffect(() => {
    ...
    const COLUMN_COUNT = Math.min(Math.max(Math.floor(width / 32), 22), 50)
    const FONT_SIZE = Math.min(Math.max(width / 80, 11), 20)
      ...
      columns.forEach((col) => {
        ...
        for (let i = 0; i < col.chars.length; i++) {
          ...
          if (i <= col.headIndex && i > col.headIndex - 4) {
            ...
            opacity = Math.max(0.12, 0.85 * (1 - distFromHead / 12))
          ...
          const hue = 40
          const sat = 70 + 15 * Math.sin(i * 0.3 + timestamp * 0.001)
          const light = 22 + 30 * Math.max(0, 1 - distFromHead / col.chars.length)
  ...
  return (
    <section
      ...
      style={{ minHeight: '68vh', background: '#fbfaf9' }}
    >
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 50% 40%, rgba(245, 158, 11, 0.08) 0%, transparent 60%)' }} />

      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full" />

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-14 flex flex-col items-center justify-center min-h-[68vh] text-center">
        <div className="mb-4">
          <DharmaWheel size={320} petals />
        </div>

        <h1 className="font-[var(--font-cormorant)] text-4xl md:text-6xl font-bold mb-3 text-golden-gradient" style={{ textShadow: '0 2px 40px rgba(245, 158, 11, 0.15)' }}>
          {locale === 'ru' ? 'Мудрость Будды' : "Buddha's Wisdom"}
        </h1>

        <p className="text-amber-600/50 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed" style={{ fontFamily: 'var(--font-cormorant)' }}>
          {locale === 'ru'
      ...
      <div className="absolute bottom-6 left-4 right-4 md:left-8 md:right-8 flex flex-col md:flex-row justify-between gap-3 pointer-events-none z-20">
        <div className="golden-card rounded-xl p-4 max-w-xs pointer-events-auto">
          ...
          <p className="text-amber-700/70 font-[var(--font-cormorant)] text-sm mt-1 leading-relaxed">
            {thesis.title}
          </p>
        </div>
        <div className="golden-card rounded-xl p-4 max-w-xs pointer-events-auto">
          ...
          <p className="text-amber-700/70 font-[var(--font-cormorant)] text-sm mt-1 leading-relaxed italic">

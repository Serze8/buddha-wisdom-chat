...
export default function PromoBanner({ page }: { page: string }) {
  ...
  return (
    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950/30 dark:to-indigo-950/20 border border-blue-200 dark:border-blue-800/30 rounded-2xl p-6 text-center">
      <span className="text-3xl block mb-2">{promo.icon}</span>
      <h3 className="font-[var(--font-jakarta)] text-xl font-bold text-blue-900 dark:text-blue-100 mb-1">
        {lang.title}
      </h3>
      ...
      <Link
        href="/auth"
        className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-blue-700 hover:bg-blue-600 text-white font-medium text-sm transition-colors"
      >
        {lang.cta} →
      </Link>
    </div>
